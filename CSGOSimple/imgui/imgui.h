...
struct ImVec2
{
    ...
    {
        IM_ASSERT(i <= 1);    // We very rarely use this [] operator, the assert overhead is fine.
        return (&x)[i];
    }
...
namespace ImGui
{
    ...
    {
        IM_ASSERT(0);    // This was misleading and partly broken. You probably want to use the ImGui::GetIO().WantCaptureMouse flag instead.
        return false;
    }
...
struct ImFontAtlas
{
    ...
    struct GlyphRangesBuilder
    {
        ...
        void           SetBit(int n)
        {
            UsedChars[n >> 3] |= 1 << (n & 7);    // Set bit 'c' in the array
        }
